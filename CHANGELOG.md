...
- **Huly integration (fork-local).** Connects NanoClaw to a self-hosted [Huly](https://huly.io) workspace: a bundled first-party MCP (`container/agent-runner/src/huly-mcp/`) gives agents `huly_*` tools for issues, todos, documents, and workspace discovery, and a poll-based chunter channel adapter (`src/channels/huly.ts`) makes agents reachable from Huly chat. Enabled by three `.env` keys (`HULY_URL`/`HULY_TOKEN`/`HULY_WORKSPACE`); `setup/huly.ts` mints the bot's scoped-grant token. Unlike the skill-installed channels, this lives directly in the barrel — it's a fork-local integration. See [docs/huly-integration.md](docs/huly-integration.md).
